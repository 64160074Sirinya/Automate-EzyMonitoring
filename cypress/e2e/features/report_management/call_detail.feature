Feature: Call detail Report

# Scenario: case1: ทดสอบการเข้าหน้าจอ Call Detail Record Report
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     Then Click menu call detail report record

# Scenario: case1: ทดสอบการค้นหา กรณีค้นหาตามช่วงวันของ Submision Date
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
#     And Select start date and end date of submission date
#     Then Click search and result is show data follow by submission date

# Scenario: case1: ทดสอบการค้นหา กรณีค้นหาตามบริษัท
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
#     And Select company
#     Then Click search and result is shoe data follow by company

# Scenario: case1: ทดสอบการค้นหา กรณีค้นหาตามสาขาของบริษัท
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
#     And Select comapany and branch
#     Then Click search and result is show data follow by company and branch

# Scenario: case1: ทดสอบการค้นหา กรณีค้นหาตาม Type Declaration
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
#     And Select type declaration
#     Then Click search and result is show data follow by type declaration

Scenario: case1: ทดสอบการดาวน์โหลดไฟล์ Excel กรณีดาวน์โหลดข้อมูลตามช่วงวัน Submission Date
    Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
    When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
    And Click Button login
    And Click module report management
    And Click menu call detail report record
    And Select start date and end date of submission date1
    And Click search and result is show data follow by submission date
    Then Click dowload excel and report show data follow by submission date

# Scenario: case1: ทดสอบการดาวน์โหลดไฟล์ Excel กรณีดาวน์โหลดข้อมูลตามบริษัทที่เลือก
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
    # And Select company
    # And Click search and result is show data follow by company
    # Then Click dowload excel and report show data follow by company

# Scenario: case1: ทดสอบการดาวน์โหลดไฟล์ Excel กรณีดาวน์โหลดข้อมูลตามสาขาของบริษัทที่เลือก
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
    # And Select company and branch
    # And Click search and result is show data follow by company and branch
    # Then Click dowload excel and report show data follow by company and branch

# Scenario: case1: ทดสอบการดาวน์โหลดไฟล์ Excel กรณีดาวน์โหลดข้อมูลตาม Type Declaration ที่เลือก
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
    # And Select type declaration
    # And Click search and result is show data follow by type declaration
    # Then Click dowload excel and report show data follow by type declaration

# Scenario: case1: ทดสอบการดาวน์โหลดไฟล์  Excel กรณีดาวน์โหลดข้อมูลตาม Submission Date, Company, Branch ที่เลือก
#     Given Open Website "http://ezymonitoring-test.tiffaedi.com:6699/"
#     When Input user "s.wittaya.ttt05@gmail.com" and password "123456"
#     And Click Button login
#     And Click module report management
#     And Click menu call detail report record
    # And Select Submission Date, Company and Branch
    # And Click search and result is show data follow by Submission Date, Company and Branch
    # Then Click dowload excel and report show data follow by Submission Date, Company and Branch